...
export default function TrashPage() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const folderRes = await getFolderByType("TRASH");
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-card">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search trash..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-earth-gray">
            <Trash2 className="h-12 w-12 mb-3 text-soil-sand" />
