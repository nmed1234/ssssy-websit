...
import { Send, Search, ChevronLeft, ChevronRight, Loader2 } from "lucide-react";
...
export default function SentPage() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const folderRes = await getFolderByType("SENT");
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-card">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search sent..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-earth-gray">
            <Send className="h-12 w-12 mb-3 text-soil-sand" />
          ...
          filtered.map((msg) => (
            <div key={msg.id}
              ...
              <span className="w-36 text-sm truncate flex-shrink-0 text-earth-gray">To: {msg.recipients?.find(r => r.recipientType === "TO")?.address || "Unknown"}</span>
