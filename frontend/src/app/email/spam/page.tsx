...
export default function SpamPage() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const folderRes = await getFolderByType("SPAM");
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-card">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search spam..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-earth-gray">
            <AlertTriangle className="h-12 w-12 mb-3 text-soil-sand" />
            <p className="text-lg font-medium">No spam</p>
            <p className="text-sm">Your spam folder is clean</p>
          </div>
        ) : (
          filtered.map((msg) => (
            <div key={msg.id}
